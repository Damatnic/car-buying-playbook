        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-surface-2 text-xs uppercase tracking-wider text-text-dim">
              <th className="sticky left-0 bg-surface-2 px-4 py-3 text-left">Quick Stats</th>
              {saved.map(v => (
                <th key={v.id} className="min-w-[160px] px-3 py-3 text-center">
                  <div className="space-y-1">
                    {v.images.exterior && (
                      <div className="relative mx-auto h-16 w-24 overflow-hidden rounded">
                        <Image src={v.images.exterior} alt={v.name} fill sizes="96px" className="object-cover" />
                      </div>
                    )}
                    <div className="font-bold text-text">{v.name}</div>
                  </div>
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {[
              ...
              <tr key={row.label} className={cn('border-b border-border/40', i % 2 === 1 && 'bg-bg/30')}>
                <td className="sticky left-0 bg-surface px-4 py-2 text-xs font-semibold text-text-dim">{row.label}</td>
                {saved.map(v => (
                  <td key={v.id} className="px-3 py-2 text-center text-sm">{row.get(v)}</td>
                ))}
              </tr>
            ))}
          </tbody>
