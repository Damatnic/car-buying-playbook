  ...
  return (
    <div>
      ...
      <header className="mb-6">
        <div className="mb-2 flex items-center gap-2 text-sm text-text-faint">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-3xl font-extrabold sm:text-4xl">{vehicle.name}</h1>
            <p className="mt-1 text-text-dim">{vehicle.trim}</p>
          </div>
          <SaveButton vehicleId={vehicle.id} />
        </div>
        <div className="mt-3 text-3xl font-extrabold tabular-nums text-accent-2">
          {fmtCurrencyRange(vehicle.priceLow, vehicle.priceHigh)}
        </div>
        <p className="mt-3 max-w-3xl text-text-dim">{vehicle.note}</p>
      </header>
